...
for REPO in ${REPOS[@]}; do
  ...
  # Download project doc files (readme & additionnal)
  ...
  if [ "$(curl --write-out '%{http_code}' --silent --output /dev/null https://github.com/$USER/$REPO/tree/$BRANCH/docs)" != '404' ]; then
    tmp/clone-subdir.sh \
      -b "main" \
      -d \
      -o "src/projects/$REPO" \
  ...
  addGithubInfos "https://github.com/$USER/$REPO" "src/projects/$REPO/sources.md"
  ...
  for FILE in $(find src/projects/$REPO -type f -name "*.md" ! -name "readme.md" ! -name "sources.md"); do
